...
export const AttackTimelineView: React.FC = () => {
  ...
  const events = currentStage.timelineEvents
  ...
  const handleAskAIAboutEvent = (event: AttackTimelineEvent, e: React.MouseEvent) => {
    e.stopPropagation()
    toggleOpen()
    sendMessage(`Analyze attack timeline stage ${event.stageNumber} (${event.title}) detected at ${event.timeStr} on ${event.deviceId}.`)
  }
    ...
    <div className="space-y-6">
      <Card variant="cyber" className="rounded-xl overflow-hidden shadow-2xl">
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          <div>
            <CardTitle className="text-sm">Chronological Attack Progression & Forensic Timeline</CardTitle>
            <p className="text-xs text-slate-400">
              Interactive multi-stage behavioral timeline correlated by SentinelX pattern correlation engine.
            </p>
          </div>
        ...
        <CardContent className="p-4 sm:p-6">
          {events.length === 0 ? (
            <div className="py-12 text-center text-xs text-slate-400">
              ...
              <p className="text-xs text-slate-500 mt-1">Use the Demo Scenario bar to step forward through the compromise timeline.</p>
            </div>
          ) : (
                    ...
                    >
                      ...
                      <div className="mt-3 pt-2.5 border-t border-slate-800/60 flex flex-wrap items-center justify-between gap-2 text-xs">
                        <div className="flex items-center gap-3 text-[11px] text-slate-400 font-mono">
                          <span>Target: <strong className="text-slate-200">{evt.deviceId}</strong></span>
                          <span>Confidence: <strong className="text-orange-400">{evt.confidenceScore}%</strong></span>
                        </div>
